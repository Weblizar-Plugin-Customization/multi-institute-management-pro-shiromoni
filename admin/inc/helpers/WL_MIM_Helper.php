...
class WL_MIM_Helper {
	...
	public static function get_sms_templates() {
		return array(
			...
			'reminder_two_days'         => esc_html__('Reminder Before two days (EMI)', WL_MIM_DOMAIN),
			'reminder_three_days'       => esc_html__('Reminder After Three days (EMI)', WL_MIM_DOMAIN),
	...
	public static function send_emi_two_day_reminder() {
		...
		foreach ($institues as $institute) {
			...
			if ($ms_student_reminder_two_days['enable'] && !empty($ms_student_reminder_two_days['message'])) {
				...
				// send sms to student before two days.
				$data = $wpdb->get_results("SELECT s.first_name, s.last_name, s.phone, i.invoice_number, i.due_date
	...
	public static function send_emi_three_days_reminder() {
		...
		foreach ($institues as $institute) {
			...
			/* Get SMS template */
			$sms_student_reminder_three_days = WL_MIM_SettingHelper::sms_template_student_reminder_three_days($institute_id);
			...
			if ($sms_student_reminder_three_days['enable'] && !empty($sms_student_reminder_three_days['message'])) {
				...
				$data = $wpdb->get_results("
					SELECT s.first_name, s.last_name, s.phone, i.invoice_number, i.due_date
					FROM {$wpdb->prefix}wl_min_students s
					JOIN {$wpdb->prefix}wl_min_invoices i ON s.id = i.student_id
	...
	public static function get_data() {
		...
		$sql          = "SELECT
		  ...
		  ( SELECT COUNT(*) FROM {$wpdb->prefix}wl_min_invoices WHERE institute_id = $institute_id ) as installments,
		...
		$students              = $wpdb->get_results("SELECT id FROM {$wpdb->prefix}wl_min_students WHERE is_deleted = 0 AND is_active = 1 AND institute_id = $institute_id");
		...
		$installments = $wpdb->get_results("SELECT paid_amount FROM {$wpdb->prefix}wl_min_installments WHERE is_deleted = 0 AND institute_id = $institute_id");
		$revenue      = 0;
		if (count($installments)) {
			foreach ($installments as $installment) {
				$revenue += $installment->paid_amount;
			}
		}
	...
	public static function get_reminder_page_url() {
		return admin_url('admin.php?page=' . 'multi-institute-management-reminders');
	}
